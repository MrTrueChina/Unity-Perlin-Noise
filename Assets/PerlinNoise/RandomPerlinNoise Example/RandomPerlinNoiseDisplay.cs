...
{
    public class RandomPerlinNoiseDisplay : MonoBehaviour
    {
        ...
        float[,] GetNoiseMap(float time)
        {
            ...
            for (int x = 0; x < _sideLength; x++)
                for (int y = 0; y < _sideLength; y++)
                    noiseMap[x, y] = RandomPerlinNoise.GetPerlinValue(x / _scale + _offset.x, y / _scale + _offset.y, _offset.z + _speed * time, _octaves, _persistence, _lacunarity);
