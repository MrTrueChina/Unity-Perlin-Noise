...
public class ArrayExamplePerlinNoiseDisplay : MonoBehaviour
{
    ...
    {
        float[,] noiseMap = GetNioseMap(time);
    ...
    float[,] GetNioseMap(float time)
    {
        float[,] noiseMap = new float[_sideLength, _sideLength];
        for (int x = 0; x < _sideLength; x++)
            for (int y = 0; y < _sideLength; y++)
                noiseMap[x, y] = ArrayExamplePerlinNoise.GetPerlinValue(x / _scale + _offset.x, y / _scale + _offset.y, _offset.z + _speed * time, _octaves, _persistence, _lacunarity);
